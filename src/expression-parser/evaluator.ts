...
function evaluateLogical(
  ...
  switch (expr.operator) {
    case "AND":
      return evaluateCondition(expr.left, context) && evaluateCondition(expr.right, context);
    case "OR":
      return evaluateCondition(expr.left, context) || evaluateCondition(expr.right, context);
    default:
  ...
  switch (expr.name) {
    ...
    case "contains": {
      ...
      if (Array.isArray(attrValue)) {
        return attrValue.some((v) => JSON.stringify(v) === JSON.stringify(searchValue));
      }
    ...
    case "size": {
      ...
      const compareNum = typeof compareValue === "number" ? compareValue : parseInt(compareValue?.N || "0");

      // operator is the comparison operator token
      switch (operator.type === "value" ? operator.value : operator) {
        case "=": return size === compareNum;
        case "<>": return size !== compareNum;
        case "<": return size < compareNum;
        case ">": return size > compareNum;
        case "<=": return size <= compareNum;
