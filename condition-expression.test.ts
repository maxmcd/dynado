...
describe("Condition Expression Evaluator", () => {
  describe("Basic Comparisons", () => {
    test("should handle = operator with strings", () => {
      ...
      expect(
        evaluateConditionExpression(item, "name = :val", undefined, { ":val": { S: "Alice" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "name = :val", undefined, { ":val": { S: "Bob" } })
      ).toBe(false);
    });
    ...
    test("should handle <> operator (not equals)", () => {
      ...
      expect(
        evaluateConditionExpression(item, "status <> :val", undefined, { ":val": { S: "inactive" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "status <> :val", undefined, { ":val": { S: "active" } })
      ).toBe(false);
    });

    test("should handle numeric comparisons", () => {
      const item: DynamoDBItem = { age: { N: "25" } };
      expect(evaluateConditionExpression(item, "age > :val", undefined, { ":val": { N: "20" } })).toBe(true);
      expect(evaluateConditionExpression(item, "age < :val", undefined, { ":val": { N: "30" } })).toBe(true);
      expect(evaluateConditionExpression(item, "age >= :val", undefined, { ":val": { N: "25" } })).toBe(true);
      expect(evaluateConditionExpression(item, "age <= :val", undefined, { ":val": { N: "25" } })).toBe(true);
      expect(evaluateConditionExpression(item, "age > :val", undefined, { ":val": { N: "30" } })).toBe(false);
    });
  });
  ...
  describe("Existence Functions", () => {
    test("should handle attribute_exists for existing attribute", () => {
      ...
      expect(evaluateConditionExpression(item, "attribute_exists(name)")).toBe(true);
      expect(evaluateConditionExpression(item, "attribute_exists(age)")).toBe(true);
    });

    test("should handle attribute_exists for non-existing attribute", () => {
      const item: DynamoDBItem = { name: { S: "Alice" } };
      expect(evaluateConditionExpression(item, "attribute_exists(missing)")).toBe(false);
    });

    test("should handle attribute_not_exists for non-existing attribute", () => {
      const item: DynamoDBItem = { name: { S: "Alice" } };
      expect(evaluateConditionExpression(item, "attribute_not_exists(missing)")).toBe(true);
      expect(evaluateConditionExpression(item, "attribute_not_exists(name)")).toBe(false);
    });

    test("should handle attribute_not_exists for null item", () => {
      expect(evaluateConditionExpression(null, "attribute_not_exists(id)")).toBe(true);
    });

    test("should handle attribute_exists with expression attribute names", () => {
      const item: DynamoDBItem = { status: { S: "active" } };
      expect(
        evaluateConditionExpression(item, "attribute_exists(#s)", { "#s": "status" })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "attribute_not_exists(#s)", { "#s": "status" })
      ).toBe(false);
    });
  });
  ...
  describe("String Functions", () => {
    test("should handle begins_with function", () => {
      ...
      expect(
        evaluateConditionExpression(item, "begins_with(prefix, :val)", undefined, {
          ":val": { S: "PROD" },
        })
      ).toBe(true);
      expect(
    ...
    test("should handle contains function with strings", () => {
      const item: DynamoDBItem = { description: { S: "This is a test description" } };
      expect(
        evaluateConditionExpression(item, "contains(description, :val)", undefined, {
          ":val": { S: "test" },
        })
      ).toBe(true);
      expect(
    ...
    test("should handle contains with missing attribute", () => {
      const item: DynamoDBItem = { name: { S: "Alice" } };
      expect(
        evaluateConditionExpression(item, "contains(missing, :val)", undefined, {
          ":val": { S: "test" },
        })
      ).toBe(false);
    });
  ...
  describe("Size Function", () => {
    test("should handle size function with strings", () => {
      const item: DynamoDBItem = { name: { S: "Alice" } };
      expect(evaluateConditionExpression(item, "size(name) = :len", undefined, { ":len": 5 })).toBe(true);
      expect(evaluateConditionExpression(item, "size(name) > :len", undefined, { ":len": 3 })).toBe(true);
      expect(evaluateConditionExpression(item, "size(name) < :len", undefined, { ":len": 10 })).toBe(true);
    });
    ...
    test("should handle size with missing attribute", () => {
      const item: DynamoDBItem = { name: { S: "Alice" } };
      expect(evaluateConditionExpression(item, "size(missing) > :len", undefined, { ":len": 0 })).toBe(false);
    });
  });
  ...
  describe("Logical Operators", () => {
    ...
    test("should handle NOT operator", () => {
      ...
      expect(
        evaluateConditionExpression(item, "NOT deleted = :true", undefined, { ":true": { S: "true" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "NOT deleted = :false", undefined, { ":false": { S: "false" } })
      ).toBe(false);
    });
    ...
    test("should handle complex AND/OR combinations", () => {
      ...
      expect(
        evaluateConditionExpression(
          ...
          undefined,
          { ":age": { N: "18" }, ":admin": { S: "admin" }, ":mod": { S: "moderator" } }
        )
      ).toBe(true);
    });
  });
  ...
  describe("Edge Cases", () => {
    test("should handle null item", () => {
      expect(evaluateConditionExpression(null, "attribute_not_exists(id)")).toBe(true);
      expect(evaluateConditionExpression(null, "attribute_exists(id)")).toBe(false);
    });
    ...
    test("should handle comparison with missing attribute", () => {
      const item: DynamoDBItem = { name: { S: "Alice" } };
      expect(
        evaluateConditionExpression(item, "missing = :val", undefined, { ":val": { S: "test" } })
      ).toBe(false);
    });
    ...
    test("should handle numeric string comparisons", () => {
      ...
      expect(
        evaluateConditionExpression(item, "version > :v", undefined, { ":v": { N: "1" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "version < :v", undefined, { ":v": { N: "3" } })
      ).toBe(true);
    });
    ...
    test("should handle zero values", () => {
      ...
      expect(
        evaluateConditionExpression(item, "counter = :zero", undefined, { ":zero": { N: "0" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "counter >= :zero", undefined, { ":zero": { N: "0" } })
      ).toBe(true);
    ...
    test("should handle negative numbers", () => {
      ...
      expect(
        evaluateConditionExpression(item, "temperature < :zero", undefined, { ":zero": { N: "0" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "temperature > :neg", undefined, { ":neg": { N: "-10" } })
      ).toBe(true);
    });
    ...
    test("should handle floating point numbers", () => {
      ...
      expect(
        evaluateConditionExpression(item, "price < :max", undefined, { ":max": { N: "20.00" } })
      ).toBe(true);
      expect(
        evaluateConditionExpression(item, "price > :min", undefined, { ":min": { N: "19.00" } })
      ).toBe(true);
    });
    ...
    test("should handle empty strings", () => {
      ...
      expect(
        evaluateConditionExpression(item, "emptyField = :empty", undefined, { ":empty": { S: "" } })
      ).toBe(true);
      expect(evaluateConditionExpression(item, "size(emptyField) = :zero", undefined, { ":zero": 0 })).toBe(true);
    });
    ...
    test("should handle unicode characters", () => {
      ...
      expect(
        evaluateConditionExpression(item, "unicode = :val", undefined, { ":val": { S: "你好世界" } })
      ).toBe(true);
      expect(evaluateConditionExpression(item, "size(unicode) = :len", undefined, { ":len": 4 })).toBe(true);
    });
    ...
    test("should handle nested expression attribute names", () => {
      ...
      expect(evaluateConditionExpression(item, "attribute_exists(user)")).toBe(true);
    });
  });
  ...
  describe("Complex Scenarios", () => {
    test("should handle multiple conditions with different operators", () => {
      ...
      expect(
        evaluateConditionExpression(
          ...
          undefined,
          { ":minAge": { N: "25" }, ":maxAge": { N: "35" }, ":active": { S: "active" } }
        )
      ).toBe(true);
    });
    ...
    test("should handle NOT with function calls", () => {
      ...
      expect(evaluateConditionExpression(item, "NOT attribute_not_exists(id)")).toBe(true);
    });
